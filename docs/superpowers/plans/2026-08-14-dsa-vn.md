...
Foreign data reaches us through **`price_board` columns instead**, and only on the `vci` source. `vnstock/explorer/vci/const.py` maps these three:

| Column | Meaning |
|---|---|
| `foreign_volume` | Foreign traded volume (snapshot) |
| `foreign_room` | Total foreign room |
| `foreign_holding_room` | Remaining foreign holding room |

**Consequence for `foreign_flow.yaml`:** it evaluates a **point-in-time snapshot** of foreign volume and remaining room — not a historical net-foreign-buy/sell time series, which the free package cannot provide. Task 1 confirms the exact column names against the live API; the strategy prompt must be written to that reality, not to a flow-over-time framing.
...
- Produces:
  - `TickerData` dataclass: `symbol: str`, `exchange: Exchange`, `history: pandas.DataFrame`, `board: dict[str, Any]`, `reference_price: float | None`, `foreign_volume: float | None`, `foreign_room: float | None`, `foreign_holding_room: float | None`
...
> **Before writing this task, read `docs/vnstock-probe-results.md` from Task 1** and use the exact column names it recorded. The names below (`time/open/high/low/close/volume`) are the expected shape; if the probe recorded different ones, use the probe's.
        ...
        [
            {
                ...
                "ref_price": 130.0,
                "match_price": 131.5,
                "foreign_volume": 500_000.0,
                "foreign_room": 49.0,
                "foreign_holding_room": 12.5,
            }
        ]
    )
...
def test_fetch_ticker_returns_populated_tickerdata():
    ...
    assert data.reference_price == pytest.approx(130.0)
    assert data.foreign_volume == pytest.approx(500_000.0)
    assert data.foreign_holding_room == pytest.approx(12.5)


def test_fetch_ticker_uses_vci_source():
    with (
        patch("data_provider.vnstock_fetcher.Quote") as quote_cls,
        patch("data_provider.vnstock_fetcher.Trading") as trading_cls,
    ):
...
def test_fetch_ticker_tolerates_missing_price_board():
    ...
    assert data.foreign_volume is None
...
Create `data_provider/__init__.py` (empty file).
...
class TickerData:
    ...
    foreign_volume: float | None
    foreign_room: float | None
    foreign_holding_room: float | None
...
def _fetch_board_row(symbol: str) -> dict[str, Any]:
    ...
    try:
        board = Trading(source=SOURCE).price_board(symbols_list=[symbol])
    ...
    # Flatten MultiIndex columns if the source returns them.
...
def fetch_ticker(symbol: str, exchange: Exchange, days: int = 200) -> TickerData:
    ...
    return TickerData(
        ...
        history=history,
        board=row,
        reference_price=_first_present(row, "ref_price", "reference_price"),
        foreign_volume=_first_present(row, "foreign_volume", "foreign_total_volume"),
        foreign_room=_first_present(row, "foreign_room", "foreign_total_room"),
        foreign_holding_room=_first_present(row, "foreign_holding_room"),
    )
...
def history_summary(data: TickerData, recent_rows: int = 10) -> str:
    ...
    if data.foreign_volume is not None:
        lines.append(f"Khối lượng khối ngoại (snapshot): {data.foreign_volume:,.0f}")
    if data.foreign_room is not None:
        lines.append(f"Room ngoại tổng: {data.foreign_room:,.2f}")
    if data.foreign_holding_room is not None:
        lines.append(f"Room ngoại còn lại: {data.foreign_holding_room:,.2f}")
...
```bash
git add data_provider/ tests/test_vnstock_fetcher.py
git commit -m "feat: add vnstock data provider pinned to VCI source"
```
...
persona: >-
  ...
  Việt Nam. Bạn hiểu rằng room ngoại còn lại thấp là yếu tố hỗ trợ giá, và bạn
  rất thận trọng khi dữ liệu chỉ là ảnh chụp tại một thời điểm.
prompt: |
  Nhiệm vụ: đánh giá tín hiệu từ dữ liệu khối ngoại.

  {market_rules}

  {data_summary}

  LƯU Ý QUAN TRỌNG VỀ DỮ LIỆU: các số liệu khối ngoại ở trên là ẢNH CHỤP TẠI
  MỘT THỜI ĐIỂM, không phải chuỗi dữ liệu mua/bán ròng theo thời gian. Bạn
  KHÔNG được suy diễn xu hướng mua ròng hay bán ròng nhiều phiên từ dữ liệu này.

  Hãy xem xét:
  - Khối lượng khối ngoại so với tổng khối lượng phiên gần nhất?
  - Room ngoại còn lại nhiều hay ít so với room tổng?
