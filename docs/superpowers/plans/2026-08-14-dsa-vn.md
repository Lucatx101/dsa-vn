...
- **Do not re-run `pip install -e .` when a task adds a new top-level package or module.** `pyproject.toml` sets `pythonpath = ["."]` under `[tool.pytest.ini_options]` specifically so `pytest` finds every new package (`data_provider`, `agent`, `notification_sender`, `formatters.py`, `pipeline.py`, `main.py`, ...) without reinstalling. The editable install's finder snapshots a static package map at install time — verified empirically that anything created afterward silently fails to import under the real `pytest` command otherwise. If a test run reports `ModuleNotFoundError` for a package you just created, check this setting is intact before doing anything else; don't work around it with a reinstall or a local `sys.path` hack.
...
- Produces:
  ...
  - `ceiling_price(reference_price: float, exchange: Exchange) -> float` — **approximate**, see note below
  - `floor_price(reference_price: float, exchange: Exchange) -> float` — **approximate**, see note below
...
**Known limitation you must document, not fix:** the exchange rounds published bands to the tick size, so `ref × (1 ± band)` is an approximation. Measured against the live board (see `docs/vnstock-probe-results.md`): FPT ref 69,200 → computed ceiling 74,044 but published 74,000; VNM ref 61,600 → computed 65,912 but published 65,900. Off by up to roughly half a tick.

Do **not** implement tick rounding — tick size varies by price level and exchange, and Task 3 fetches the exchange's own `listing_ceiling` / `listing_floor`, which are authoritative. Your job is to make the approximation explicit: name it in the docstrings of `ceiling_price` / `floor_price`, and state in `market_rules_text()` that computed bands are indicative and the published trần/sàn wins.
...
- Produces:
  ...
  - `TickerData` dataclass: `symbol: str`, `exchange: Exchange`, `history: pandas.DataFrame`, `board: dict[str, Any]`, `reference_price: float | None`, `board_ceiling: float | None`, `board_floor: float | None`, `foreign_buy_volume: float | None`, `foreign_sell_volume: float | None`, `current_room: float | None`, `total_room: float | None`, plus a `foreign_net_volume` property returning `buy − sell` or `None`
...
> **Read `docs/vnstock-probe-results.md` (written by Task 1) before starting.** Every column name you need is now settled there and in "Live probe results" above — including the reference-price question, which a targeted re-probe resolved: the `listing_*` and `match_*` variants carry identical values, and we use the `listing_*` group.
>
> The board's `listing_ceiling` / `listing_floor` are the **exchange's published** bands and are authoritative. `core.market_profile.ceiling_price()` / `floor_price()` are tick-rounding approximations and are only a fallback — capture the published pair into `TickerData` so `price_band_risk` can prefer it.
